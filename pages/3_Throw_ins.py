...
if df.empty:
    st.warning("No throw-in rows were found in Data/SWE SP.xlsx or Data/Czech SP.xlsx.")
    st.stop()
...
final_third_share = float((sequences["Zone"] == "Final-third pressure").mean() * 100) if not sequences.empty else 0.0
long_throw_share = float((sequences["Profile"] == "Long throw threat").mean() * 100) if not sequences.empty else 0.0
best_seq_xg = float(sequences["Total xG"].max()) if not sequences.empty else 0.0

c1, c2, c3, c4 = st.columns(4)
c1.metric("Throw-in sequences", seq_count)
...
with overview_tab:
    ...
    if not sequences.empty:
        ...
        top_profile = sequences["Profile"].value_counts().head(1)
        top_side = sequences["Side"].value_counts().head(1)
        if not top_zone.empty:
            insights.insert(0, f"Most common territory is {top_zone.index[0].lower()} ({top_zone.iloc[0]} sequences).")
